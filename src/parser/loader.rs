...
pub(crate) fn load_from_url(url: reqwest::Url, syntax: Option<Syntax>) -> crate::Result<RawObject> {
    if url.scheme() == "file" {
        let path = url.to_file_path().unwrap();
        load_from_file(path, syntax)
    } else {
...
fn load_hocon(s: &str) -> crate::Result<RawObject> {
    let (_, raw_object) = parse(s).unwrap();
    Ok(raw_object)
}
